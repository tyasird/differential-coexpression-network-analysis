...
class Graph(GraphBase):
    ...
    def all_st_mincuts(self, source, target, capacity=None):
        ...
        return [Cut(self, value, cut=cut, partition=part)
                for cut, part in zip(cuts, parts)]
    ...
    def write_dimacs(self, f, source=None, target=None, capacity="capacity"):
        ...
        if isinstance(capacity, str) and \
    ...
    def write_svg(self, fname, layout="auto", width=None, height=None, \
        ...
        if any(x == 3 for x in shapes):
            ...
            tk_window = tk.Tk()
            font = tkinter.font.Font(root=tk_window, font=("Sans", font_size_in_pixels, tkinter.font.NORMAL))
        else:
            tk_window = None
        ...
        for vidx in range(self.vcount()):
            ...
            print('      <text sodipodi:linespacing="125%" y="{0}" x="0" id="text{1}" style="font-size:{2};font-style:normal;font-weight:normal;text-align:center;line-height:125%;letter-spacing:0px;word-spacing:0px;text-anchor:middle;fill:#000000;fill-opacity:1;stroke:none;font-family:Sans">'.format(vertex_size / 2.,vidx, font_size), file=f)
            print('<tspan y="{0}" x="0" id="tspan{1}" sodipodi:role="line">{2}</tspan></text>'.format(vertex_size / 2.,vidx, str(labels[vidx])), file=f)
            print('    </g>', file=f)
    ...
    def DictList(klass, vertices, edges, directed=False, \
        ...
        if iterative:
            ...
            if len(vertex_name_map) > n:
                ...
                more = [None] * diff
                for k, v in vertex_attrs.items(): v.extend(more)
